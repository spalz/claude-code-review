...
function getHooksDir(workspacePath) {
    return path.join(workspacePath, ".claude", "hooks");
}

function getHookPath(workspacePath) {
 ...
 */
function isHookInstalled(workspacePath) {
    ...
    if (actual !== expected) {
        log.log(`isHookInstalled: script content mismatch (expected ${expected.length} chars, got ${actual.length})`);
        return false;
    }
    ...
    try {
        ...
        log.log(`isHookInstalled: settings read error: ${err.message}`);
        return false;
    }
...
function installHook(workspacePath) {
...
function registerHookInSettings(workspacePath, hookPath) {
    const settingsPath = getClaudeSettingsPath(workspacePath);
    let settings = {};
    try {
    ...
    settings.hooks.PostToolUse = settings.hooks.PostToolUse.filter((entry) => {
        ...
        if (entry.matcher && Array.isArray(entry.hooks) &&
 ...
 */
function checkAndPrompt(workspacePath, onStatusChange) {
    ...
    const status = exists ? "outdated" : "missing";
...
function doInstall(workspacePath, onStatusChange) {
    try {
        ...
        log.log(`doInstall error: ${err.message}`);
        vscode.window.showErrorMessage(
            `Failed to install hook: ${err.message}`,
        );
    }
}

module.exports = { checkAndPrompt, installHook, isHookInstalled, getHookPath, doInstall };
