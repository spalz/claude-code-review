...
const PORT = 27182;
let server = null;
let _addFileToReview = null;

function setAddFileHandler(fn) {
    _addFileToReview = fn;
}
        ...
        if (req.method === "POST" && req.url === "/changed") {
            readBody(req, (body) => {
                try {
                    ...
                    log.log(`/changed error: ${err.message}`);
                }
                json(res, { ok: true });
            });
            return;
...
module.exports = { startServer, stopServer, setAddFileHandler };
